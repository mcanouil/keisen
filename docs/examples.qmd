...
Every table on this page is rendered from a file in the repository's `tests/visual/` directory, and the image beside each listing is that file's own output.
Nothing here is illustrative: if the code drifted from the picture, the test suite would fail.
...
```typst
#import "@preview/keisen:0.1.0": *
#import "@preview/keisen:0.1.0/src/integrations/gribouille.typ": nanoplot-bar, nanoplot-line
...
#display-table(
  ...
  format-nanoplot("trend", plot: nanoplot-line, width: 8em, height: 3.2em, values: trends),
  format-nanoplot("volume", plot: nanoplot-bar, width: 8em, height: 3.2em, values: trends),
...
Cash reads as flat because all three rows share one scale.
Scaled per cell, its small wobble would draw exactly like the others' real movement, which is how a nanoplot lies.

The renderers live in `src/integrations/gribouille.typ`, the only file in the package that imports another package.
A document that never imports it never fetches `gribouille`.
